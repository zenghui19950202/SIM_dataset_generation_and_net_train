...
def reconstruction(SIM_data, SIM_pattern, input_num=5, image_show=True):
    ...
    if experimental_params.upsample == True:
        ...
        input_SIM_raw_data = up_sample(input_SIM_raw_data)
    else:
    ...
    SR_result = SR_reconstruction(SR_image, input_SIM_raw_data, optimizer_SR_and_polarization,
...
def SR_reconstruction(SR_image, input_SIM_raw_data, optimizer_SR_and_polarization,
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(estimated_pattern_parameters.size()[0]):
            ...
            SIM_raw_data_estimated = forward_model.complex_stack_to_intensity(torch.ifft(forward_model.torch_2d_ifftshift(forward_model.torch_2d_fftshift(
                torch.fft((sample_light_field_complex), 2)) * OTF.unsqueeze(2) ) ,2) ).float()

            mse_loss = criterion(SIM_raw_data_estimated.unsqueeze(0).unsqueeze(0), input_SIM_raw_data[:, i, :, :].unsqueeze(0))
            loss += mse_loss
...
if __name__ == '__main__':
    ...
    SSIM, PSNR, best_SR = reconstruction(SIM_data, SIM_pattern, input_num=9)
    if best_SR.dim() == 3:
