...
def reconstruction(SIM_data, SIM_pattern, input_num=5, image_show=True):
    ...
    CTF = experimental_params.CTF_form(fc_ratio=1)
    ...
    input_SIM_raw_data_fft = unsample_process(image_size, input_SIM_raw_data, CTF, OTF, experimental_params.upsample)
    ...
    input_SIM_raw_data_fft = input_SIM_raw_data_fft.to(device)
...
def unsample_process(image_size, input_SIM_raw_data, CTF, OTF, upsample_flag=False):
    device = CTF.device
    ...
    if upsample_flag == True:
        ...
        for i in range(input_SIM_raw_data.size()[1]):
            ...
            SIM_raw_data_fft_deconv = SIM_raw_data_fft * OTF / (OTF * OTF + 0.04)
            input_SIM_raw_data_fft[:, i, :, :, :] = torch.stack(
                [ZeroPad(SIM_raw_data_fft_deconv[:, :, 0]), ZeroPad(SIM_raw_data_fft_deconv[:, :, 1])], 2)
    else:
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(estimated_pattern_parameters.size()[0]):
            ...
            SIM_raw_data_fft_estimated = forward_model.torch_2d_fftshift(
                torch.fft((sample_light_field_complex), 2))

            mse_loss = criterion(SIM_raw_data_fft_estimated.unsqueeze(0), input_SIM_raw_data_fft[:, i, :, :, :])
            loss += mse_loss
