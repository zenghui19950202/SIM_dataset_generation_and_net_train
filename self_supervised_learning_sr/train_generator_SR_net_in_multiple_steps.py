...
def train(net, SIM_data_loader, SIM_pattern_loader, net_input, criterion, num_epochs, device, lr=None,
    ...
    for epoch in range(num_epochs):
        ...
        if epoch > 5000:
            LR_estimated = forward_model.positive_propagate(SR_image, 1, psf_conv)
            loss += criterion(LR_estimated, LR, mask)
        else:
            LR_estimated = forward_model.positive_propagate(SR_image, 1, psf_conv)
            loss += 50 * criterion(LR_estimated, LR, mask)
            SIM_raw_data_estimated = forward_model.positive_propagate(SR_image, temp_input_SIM_pattern.detach(),
                                                                      psf_conv)
            loss += criterion(SIM_raw_data_estimated, input_SIM_raw_data, mask)
