...
def train(net, SIM_data_loader, SIM_pattern_loader, net_input, criterion, num_epochs, device, lr=None,
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(temp_input_SIM_pattern.size()[1]):
            ...
            input_SIM_raw_data_complex = torch.stack([input_SIM_raw_data[:, i, :, :].squeeze(), torch.zeros_like(input_SIM_raw_data[:, i, :, :]).squeeze()], 2)
            input_SIM_raw_data_fft = forward_model.torch_2d_fftshift(torch.fft(input_SIM_raw_data_complex, 2)) * CTF.unsqueeze(2)
            mse_loss = criterion(SIM_raw_data_fft_estimated, input_SIM_raw_data_fft, 1)
            loss += mse_loss
        ...
        if (epoch + 1) % num_epochs == 0:
            ...
            result = processing_utils.notch_filter_for_all_vulnerable_point(SR_image_high_freq_filtered, estimated_pattern_parameters).squeeze()[
                     psf_radius: -psf_radius, psf_radius: -psf_radius]
