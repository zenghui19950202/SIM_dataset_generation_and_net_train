...
class SIM_data(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        SR_image_directoty = SIM_data_directory \
                             + "_SR_" \
                             + '.' + image_format
        SR_image_PIL = Image.open(SR_image_directoty)

        if len(SR_image_PIL.size) == 2:
            image_size = [SR_image_PIL.size[0],SR_image_PIL.size[1]]
        elif len(SR_image_PIL.size) == 3:
            image_size = [SR_image_PIL.size[1], SR_image_PIL.size[2]]

        transform = transforms.Compose(
            [transforms.ToTensor(),  # 函数接受PIL Image或numpy.ndarray，将其先由HWC转置为CHW格式，再转为float后每个像素除以255.
             transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])

        SR_normalized_image_tensor = transform(SR_image_PIL)[0,:,:]
        ...
        SIM_image_data[image_number,:,:] =  appro_widefield_image # add one layer of approximately wide field image(mean of all the SIM)

        return SIM_image_data,SR_normalized_image_tensor
